  ...
  return (
    ...
    >
      <header className="mb-4 shrink-0">
        ...
        <p className="mt-1 text-sm text-gray-600">오늘 일과 5줄</p>
      </header>

      <ol className="flex flex-1 flex-col justify-between gap-2" role="list">
        {tasks.map((task, index) => (
          <li
            key={task.id}
            className="flex min-h-[3rem] items-center gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-sm"
          >
            <input
              id={`task-${task.id}`}
              type="checkbox"
